...
public class Zombie : LivingEntity
{
    ...
    public override void OnDamage(float damage, Vector3 hitPoint, Vector3 hitNormal) {
        ...
        if(!dead)
        {
            hitEffect.transform.position = hitPoint; //(피격위치)
            hitEffect.transform.rotation = Quaternion.LookRotation(hitNormal); //공격이 날라온 방향을 바라보는 방향(피격방향)
            hitEffect.Play();
